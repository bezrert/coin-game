...
- **Готовый примитив проверяемой случайности.** Chainlink VRF v2.5 даёт случайность с
  криптографическим доказательством прямо on-chain. Для требования «параноик с блок-эксплорером
  убеждается, что это не скам» это идеально: исход выводится из числа, которое нельзя подделать.
  На Solana пришлось бы брать Switchboard/ORAO VRF — рабочий, но история верифицируемости через
  обычный эксплорер менее «из коробки».
...
- `payoutBps`/лимиты меняет только owner; `subscriptionId`/`callbackGasLimit` — только передеплоем.

## Самый сложный unknown, который удалось разгадать

Ставка разрешалась on-chain (событие `BetSettled` есть в блоке), но UI **бесконечно висел на
«Ждём VRF»**. По логам RPC обнаружилось, что фронт долбит `eth_newFilter` десятки раз подряд и
никогда не получает результат. Причина: `useWatchContractEvent` в poll-режиме использует
**stateful-фильтры** (`eth_newFilter` + `eth_getFilterChanges`), а публичный балансируемый RPC
**не хранит фильтр между запросами** — каждый опрос уходит на другую ноду, фильтр там неизвестен,
viem молча пересоздаёт его по кругу. Событие не доставляется.

Решение — отказаться от фильтров и поллить **stateless `eth_getLogs`** (`getContractEvents` через
react-query, ключ — `requestId`, `fromBlock` = блок ставки). getLogs не зависит от того, какая
нода ответит. Попутно вскрылся второй нюанс: VRF резервирует max-стоимость фулфилмента (~80 LINK
при `callbackGasLimit = 500k`), из-за чего запросы «висли по балансу» — снизили лимит до 150k, и
резерв упал ~в 2.4 раза.
